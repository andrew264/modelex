...
class CustomFTDataset:
    def __init__(self, conv_path: str, parquet_path: str, tokenizer_path: str, num_epochs: int|str=1, mix_ratio: int|str = 1) -> None:
        if isinstance(num_epochs, str): num_epochs = int(num_epochs)
        if isinstance(mix_ratio, str): mix_ratio = int(mix_ratio)
        self.conversations = Conversations(conv_path, tokenizer_path)
